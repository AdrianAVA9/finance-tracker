...
public sealed class BudgetTests
{
    ...
    public void Create_Should_Succeed_When_ParametersValid()
    {
        ...
        var result = Budget.Create("user-1", categoryId: categoryId, amount: 100m, isRecurrent: false, month: 6, year: 2025);
    ...
    {
        var result = Budget.Create("user-1", Guid.NewGuid(), 50m, false, 1, 2024);
    ...
    {
        var result = Budget.Create("user-1", Guid.NewGuid(), amount, false, 1, 2024);
    ...
    {
        var result = Budget.Create("user-1", Guid.NewGuid(), 10m, false, month, 2024);
    ...
    {
        var result = Budget.Create("user-1", Guid.NewGuid(), 10m, false, 1, year);
    ...
    {
        var budget = Budget.Create("user-1", Guid.NewGuid(), 100m, false, 1, 2024).Value;
    ...
    {
        var budget = Budget.Create("user-1", Guid.NewGuid(), 100m, false, 3, 2024).Value;
        budget.ClearDomainEvents();
